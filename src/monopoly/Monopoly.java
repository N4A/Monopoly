	...
	public static void main(String[] args) throws IOException {
		...
		while (true) {
			...
			if (players.size() == 1) {//only one player left
				ctrl.showMassage(StrSrc.congatulatePlayer.getValue() + players.get(0).getName() + StrSrc.winGame.getValue());
				break;
			}
			...
			if (dateUtil.getDay() ==  1) {//new month
				Bank.handleInterest(players, ctrl);//bank give interest
				lotterySystem.checkLotteries(players, ctrl);//lottery draw
			}
