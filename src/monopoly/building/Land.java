...
public class Land extends Creature {
	...
	private void sellLand(Player p, UIController uiController) {
		...
		uiController.showMassage(StrSrc.youReach.getValue() + StrSrc.landOnsellName.getValue());// reach
																								// on-sell
																								// land
		uiController.showMassage(StrSrc.whetherSpend.getValue() + price + StrSrc.buyland.getValue());
		uiController.showMassage(StrSrc.landInputNotice.getValue());
	...
	private void upgradeLand(Player p, UIController uiController) {
		...
		uiController.showMassage(StrSrc.youReach.getValue() + StrSrc.landOfSelf.getValue());
		uiController.showMassage(StrSrc.whetherSpend.getValue() + cost + StrSrc.upgradeLand.getValue());
		uiController.showMassage(StrSrc.landInputNotice.getValue());
		...
		while (!success) {
			...
			case "y": {
				if (p.getCash() < cost) {
					...
					level++;
					uiController
							.showMassage(StrSrc.landUpgradeSeccess.getValue() + level + StrSrc.landLevel.getValue());
					success = true;
				}
				break;
			}
