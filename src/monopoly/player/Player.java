...
public abstract class Player extends Creature {
	...
	public void execute(Player p, UIController uiController) {
		...
		StringBuffer sb = new StringBuffer(StrSrc.you.getValue() + StrSrc.meet.getValue() +
				StrSrc.playerName.getValue() + this.getName() + "\n" 
				+ StrSrc.fight.getValue()+"\n");
		int rm = (int) (Math.random() * 3);
		if (rm == 0) {
			sb.append(StrSrc.playerName.getValue()+p.getName()+StrSrc.hurt.getValue() + "\n");
			p.setHurt(2);
			map.moveToHospital(p);
		}
		else if (rm == 1) {
			sb.append(StrSrc.playerName.getValue()+getName()+StrSrc.hurt.getValue() + "\n");
			this.setHurt(2);
			map.moveToHospital(this);
		}
		else {
	...
	private Image getWalkImage(int i) {
		return type.getPlayerWalkIcon()[i].getImage();
	}
	
	public void setWalkImage(int i) {
		this.walkImage = getWalkImage(i);
	}
