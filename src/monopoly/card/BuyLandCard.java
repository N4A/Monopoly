...
public class BuyLandCard extends Card {
	...
	public void execute(Player player, Map map, UIController uiController) {
		...
		Land land;
		if (creature instanceof Land && (land = (Land)creature).getType() != LandType.none && land.getOwner() != player) {
			...
			if (player.getCash() < cost) {
				...
				player.addLand(land);
				uiController.showMassage(StrSrc.landSellSuccess.getValue() + land.getLevel() + StrSrc.landLevel.getValue());
				player.rmCard(this);
			...
			uiController.showMassage(StrSrc.buyLandCardWarning.getValue());
		}
	}
