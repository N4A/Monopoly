...
public class StockSystem {
	...
	private final String[] stocksName = { StrSrc.stock1.getValue(), StrSrc.stock2.getValue(), StrSrc.stock3.getValue(),
	...
	public String[] getStockClum(){
		return new String[]{StrSrc.id.getValue(),StrSrc.stockName.getValue(),
			StrSrc.price.getValue(),StrSrc.status.getValue(),
			StrSrc.stockLeft.getValue(),StrSrc.ownNumber.getValue()};
	}
	
	public Object[][] getStockDetial(Player player){
		Object[][] details = new Object[stocks.size()][];
		int i = 0;
		for (Stock stock : stocks) {
			Investor investor = getInvestor(player.getType());
			int number = investor.getStockNumber(stock.getName());
			details[i] = new Object[]{i+1,stock.getName(),stock.getPrice(),stock.getStatus()
					,stock.getLeftNumber(),number};
			i++;
		}
		return details;
	 ...
	 */
	public void handleStocks(Player player, UIController uiController) {
		boolean over = false;
		while (!over) {
	 ...
	 */
	public void playerSellStock(int index, int number, Player player, UIController uiController) {
	 ...
	 */
	public void playerBuyStock(int index, int number, Player player, UIController uiController) {
